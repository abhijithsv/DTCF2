#include "mpi.h"
#include "grid.h"
#include "tensor.h"
#include "contraction.h"
#include "grid_redib.h"
using namespace std;

int main(int argc, char* argv[])
{
    MPI_Init(&argc, &argv);
    int rank,np;
    MPI_Comm_rank(MPI_COMM_WORLD,&rank);

    MPI_Comm_rank(MPI_COMM_WORLD,&np);

    int size = 128;
    int block_grid = 8;
    int grid_dim = 4;
    int* pgrid = new int[grid_dim];

    
	pgrid[0] = 2;
	pgrid[1] = 2;
	pgrid[2] = 2;
	pgrid[3] = 2;
    

    if(rank==0) cout<<"Grid about to initialized"<<endl;

    Grid* grid = new Grid(grid_dim, pgrid);
	
	int grid_dim2=2;
	int* pgrid2=new int[grid_dim2];
	pgrid2[0]=4;
	pgrid2[1]=4;

	
	Grid* ng = new Grid(grid_dim2, pgrid2);

    if(rank == 0) cout<<"Grid initialized"<<endl;

    int* size_A = new int[2];
    int* block_grid_A = new int[2];
    int* idmap_A = new int[2];

    size_A[0] = size;
    size_A[1] = size;
  

    if(rank == 0) cout<<"Size "<<size<<endl;

    block_grid_A[0] = block_grid;
    block_grid_A[1] = block_grid;


    if(rank == 0) cout<<"Block Grid "<<block_grid<<endl;


    int s = 4;

	
    int* idmaps = new int[2];
    idmaps[0]=0;	idmaps[1]=1;
	
	int* idmaps2 = new int[2];
	idmaps2[0]=0; idmaps2[1]=1;

 
    Tensor* A = new Tensor("cc", idmaps, size_A, block_grid_A, grid);
    A->initialize();

    if(rank == 0) cout<<"Tensor A initialized"<<endl;


	
	
	MPI_Barrier(MPI_COMM_WORLD);	
	double time =0;
	

	    if(rank == 0) cout << "Redistributing "<< endl;


	    time=-MPI_Wtime();
	    GridRedistribute* redib = new GridRedistribute(A,idmaps,grid);
	redib->redistribute();
	    time+=MPI_Wtime();

	    MPI_Barrier(MPI_COMM_WORLD);
	    if (rank == 0) cout << "Redistribution  took : "<<time<<" seconds"<<endl;
	   
	

}
